...
export default async function EMTDashboardPage() {
  ...
  const { data: rawInvites, error: inviteErr } = await supabase
    .from("bookings")
    .select(
      "id, event_name, event_type, event_date, location, expected_attendance, duration_hours, rate_cents, notes, slot_index, invitation_expires_at, organizer:profiles!bookings_organizer_id_fkey ( full_name )"
    )
