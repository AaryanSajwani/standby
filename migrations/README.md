...
| `0015_staffing_slots.sql` → `0019_set_event_headcount.sql` | Staffing slots (Phase 1–6): addressable slots, `booking_events` audit, `invited_medic_id` held state, open-slot RLS + invited counts, unassign/reopen, `set_event_headcount` (revive/append/retire) | **Applied** (2026-08-10) | See the `staffing-slots` skill for the data model + invariants; each file carries its own verification block |
| `0020_rate_cents_expand.sql` | Money → integer cents, **EXPAND** half: backfill `rate_cents` from `offered_rate`, sanity CHECK, INSERT sync trigger keeping both fields populated during the code switch | **Applied** (2026-08-10) | No — additive + reversible; both rate fields stay valid |
| `0021_rate_cents_contract.sql` | Money → integer cents, **CONTRACT** half: redefine `set_event_headcount` to copy `rate_cents`, drop the sync trigger/function, backfill remaining nulls, `rate_cents` **SET NOT NULL**, **drop `offered_rate`** | **Applied** (2026-08-26) | ⚠ Irreversible (drops a column). Deploy the `offered_rate`-free app code FIRST — old code that still SELECTs the column 500s once it's gone. `unassign` no longer nulls `rate_cents` (the NOT NULL precondition); reopened slots keep an advertisable rate |
| `0022_emt_verification_workflow.sql` | Admin EMT credential review: `emt_profiles.verification_status` (pending/accepted/rejected) + `rejection_reason`/`reviewed_at`/`reviewed_by`, backfilled from `verified`; RESTRICTIVE insert policy pinning client rows to a clean `pending` | **Applied** (2026-08-27) | No — additive. `verified` stays THE gate; status is a parallel review state so the queue can tell pending from rejected. All writes are service-role (admin route); clients can't UPDATE the new cols (no table-level UPDATE grant) and can't self-accept at INSERT (restrictive policy) |
| `0023_emt_credential_column_grants.sql` | Close the credential-PII read gap: revoke table-wide `SELECT` on `emt_profiles` from anon+authenticated, re-grant SELECT on ONLY the 10 `EMT_PUBLIC_COLUMNS` | **Applied** (2026-08-28) | No — the deployed app only ever client-reads the public columns (credential fields go through the service role). Fixes a real live leak: any authenticated user could raw-PostgREST `select license_number … where verified=true`. service_role keeps full read |
