...
| §5 | Personnel marketplace | ✅ Shipped — live filter counts, `/for-emts` founding-medic page, verification badges, booking request → EMT accept flow, availability calendar, honest empty states. **Email notifications shipped 2026-07-22** (request → EMT, decision → organizer, via Resend) — live once migration 0006 runs + domain verifies. SMS deliberately deferred (Twilio cost + A2P overhead). (Medic recruiting = ongoing, non-code) |
| §6 | Monetization | ✅ Copy shipped — buyer-side commission + Premier "coming soon"; billing build deliberately deferred. **Repriced 2026-07-22: EMR $20–28 · EMT-B $20–40; paramedic tier removed** (BLS supply only — engine caps at EMT-B, HIGH/CRITICAL reports carry an ALS-coordination advisory) |
...
(Email notifications shipped 2026-07-22 — run migration 0006 + verify the Resend domain.)
